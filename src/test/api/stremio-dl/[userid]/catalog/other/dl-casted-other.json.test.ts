import handler from '@/pages/api/stremio-dl/[userid]/catalog/other/dl-casted-other.json';
import { createMockRequest, createMockResponse } from '@/test/utils/api';
import { getDebridLinkDMMLibrary } from '@/utils/debridLinkCastCatalogHelper';
import { beforeEach, describe, expect, it, vi } from 'vitest';

vi.mock('@/utils/debridLinkCastCatalogHelper', () => ({
	getDebridLinkDMMLibrary: vi.fn(),
}));

const mockLibrary = vi.mocked(getDebridLinkDMMLibrary);

describe('/api/stremio-dl/[userid]/catalog/other/dl-casted-other.json', () => {
	...
	it('returns the first page of the library', async () => {
		const data = {
			metas: [{ id: 'dmm-dl:t1', name: 'R', type: 'other' }],
			hasMore: true,
			cacheMaxAge: 0,
		};
