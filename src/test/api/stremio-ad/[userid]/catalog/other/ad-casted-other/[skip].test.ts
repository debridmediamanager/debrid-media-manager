...
describe('/api/stremio-ad/[userid]/catalog/other/ad-casted-other/[skip]', () => {
	...
	it('returns metas with pagination', async () => {
		const mockMetas = [{ id: 'dmm-ad:456', type: 'other', name: 'Test2' }];
		...
		await handler(req, res);
		expect(res.status).toHaveBeenCalledWith(200);
		expect(mockGetAllDebridDMMLibrary).toHaveBeenCalledWith('test-key', 24);
		const data = res._getData() as any;
		expect(data.metas).toEqual(mockMetas);
		expect(data.cacheMaxAge).toBe(0);
	});

	it('defaults page to 0 when skip is not a string', async () => {
		mockRepository.getAllDebridCastProfile = vi.fn().mockResolvedValue({ apiKey: 'test-key' });
		mockGetAllDebridDMMLibrary.mockResolvedValue([]);
