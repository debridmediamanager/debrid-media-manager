import handler from '@/pages/api/stremio-ad/[userid]/catalog/series/ad-casted-shows.json';
...
describe('/api/stremio-ad/[userid]/catalog/series/ad-casted-shows.json', () => {
	let res: ReturnType<typeof createMockResponse>;

	beforeEach(() => {
		vi.clearAllMocks();
		res = createMockResponse();
		mockRepository.fetchAllDebridCastedShows = vi.fn();
	});

	it('sets CORS header', async () => {
		mockRepository.fetchAllDebridCastedShows = vi.fn().mockResolvedValue([]);
	...
	it('returns metas with poster URLs', async () => {
		mockRepository.fetchAllDebridCastedShows = vi
			.fn()
	...
	it('returns 500 on error', async () => {
		mockRepository.fetchAllDebridCastedShows = vi.fn().mockRejectedValue(new Error('DB error'));
		const req = createMockRequest({ query: { userid: 'user1' } });
		await handler(req, res);
