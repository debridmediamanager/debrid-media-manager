import handler from '@/pages/api/stremio-pm/[userid]/catalog/other/pm-casted-other.json';
import { createMockRequest, createMockResponse } from '@/test/utils/api';
import { getPremiumizeDMMLibrary } from '@/utils/premiumizeCastCatalogHelper';
import { beforeEach, describe, expect, it, vi } from 'vitest';

vi.mock('@/utils/premiumizeCastCatalogHelper', () => ({
	getPremiumizeDMMLibrary: vi.fn(),
}));

const mockLibrary = vi.mocked(getPremiumizeDMMLibrary);

describe('/api/stremio-pm/[userid]/catalog/other/pm-casted-other.json', () => {
	...
	it('returns the first page of the library', async () => {
		const data = {
			metas: [{ id: 'dmm-pm:folder:f1', name: 'R', type: 'other' }],
			hasMore: true,
			cacheMaxAge: 0,
		};
