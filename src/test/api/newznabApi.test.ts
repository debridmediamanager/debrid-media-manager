...
/**
 * What the store holds for a release grabbed before the per-download stamp fix:
 * the rebuild of that day, which took the head, posters and dates off but kept
 * the upstream's own newsgroups. Generated by running the sanitizer as it stood
 * in `5a77de9d` over `RAW_NZB`, which is how these objects were actually
 * written — more than half the live store is still in this shape.
 */
const STALE_STORED_NZB = `<?xml version="1.0" encoding="UTF-8"?>
<nzb xmlns="http://www.newzbin.com/DTD/2003/nzb">
	<head>
		<meta type="name">Some.Release.2160p.WEB</meta>
	</head>
	<file subject="[1/1] &quot;Some.Release.2160p.WEB.part01.rar&quot; yEnc (1/2)">
		<groups>
...
/** A stored copy already cleaned by today's rules — the common case. */
const CURRENT_STORED_NZB = sanitizeNzb(RAW_NZB).xml;
...
describe('GET /api/newznab/api grab', () => {
	...
	it('serves a store hit without touching an upstream', async () => {
		mockGetStored.mockResolvedValue(CURRENT_STORED_NZB);

		const res = await run({ t: 'get', id: token(), apikey: SPONSOR_KEY });

		expect(fetchMock).not.toHaveBeenCalled();
		// Byte-identical, and not rewritten: the re-clean of a current copy must
		// be free of both an upstream call and a store write.
		expect(body(res)).toBe(CURRENT_STORED_NZB);
		expect(res._getHeaders()['X-Nzb-Removed']).toBe('-');
		expect(mockPutStored).not.toHaveBeenCalled();
	});

	it('cleans a stored copy written under older rules before serving it', async () => {
		mockGetStored.mockResolvedValue(STALE_STORED_NZB);

		const res = await run({ t: 'get', id: token(), apikey: SPONSOR_KEY });

		const xml = body(res);
		expect(fetchMock).not.toHaveBeenCalled();
		// The newsgroup list is the channel one upstream stamps per download, so
		// an object stored before that rule existed cannot be served as it lies.
		expect(xml).not.toContain('alt.binaries.test');
		expect(xml).toContain('<group>alt.binaries.misc</group>');
		expect(xml).toContain('first-article@news');
		expect(xml).toContain('<meta type="name">Some.Release.2160p.WEB</meta>');
		expect(res._getHeaders()['X-Nzb-Removed']).toContain('newsgroups');
		// And the stale object is replaced, so the next grab of it is current.
		expect(mockPutStored).toHaveBeenCalledWith('ds', 'first-native-id', xml);
	});

	it('falls back to the upstream when a stored copy cannot be cleaned at all', async () => {
		mockGetStored.mockResolvedValue('<nzb>nothing a downloader can use</nzb>');

		const res = await run({ t: 'get', id: token(), apikey: SPONSOR_KEY });

		expect(fetchedUrls()[0]).toContain('t=get');
		expect(body(res)).toContain('first-article@news');
	});
