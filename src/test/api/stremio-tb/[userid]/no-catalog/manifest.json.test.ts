import handler from '@/pages/api/stremio-tb/[userid]/no-catalog/manifest.json';
import { createMockRequest, createMockResponse } from '@/test/utils/api';
import { beforeEach, describe, expect, it } from 'vitest';

describe('/api/stremio-tb/[userid]/no-catalog/manifest.json', () => {
	...
	it('returns dev name when host does not include debridmediamanager.com', async () => {
		...
		await handler(req, res);
		const data = res._getData() as any;
		expect(data.name).toBe('[LOCAL] DMM Cast for TorBox (No Library)');
	});
