import handler from '@/pages/api/stremio-tb/[userid]/manifest.json';
import { createMockRequest, createMockResponse } from '@/test/utils/api';
import { beforeEach, describe, expect, it } from 'vitest';

describe('/api/stremio-tb/[userid]/manifest.json', () => {
	...
	it('returns correct manifest id and name', async () => {
		...
		expect(data.id).toBe('com.debridmediamanager.cast.torbox');
		expect(data.name).toBe('DMM Cast for TorBox');
	});
	...
	it('includes catalogs', async () => {
		...
		expect(data.catalogs[0].id).toBe('tb-casted-movies');
		expect(data.catalogs[1].id).toBe('tb-casted-shows');
		expect(data.catalogs[2].id).toBe('tb-casted-other');
	});
	...
	it('includes resources', async () => {
		...
		expect(data.resources[1].idPrefixes).toEqual(['dmm-tb']);
	});
