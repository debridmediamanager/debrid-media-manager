import handler, { offcloudCastManifest } from '@/pages/api/stremio-oc/[userid]/manifest.json';
import { createMockRequest, createMockResponse } from '@/test/utils/api';
import { describe, expect, it } from 'vitest';

describe('/api/stremio-oc/[userid]/manifest.json', () => {
	...
	it('leaves the library out of the no-catalog variant', () => {
		const manifest = offcloudCastManifest(false);
	...
	it('gives the no-catalog variant its own addon id', () => {
		expect(offcloudCastManifest(false).id).not.toBe(offcloudCastManifest(true).id);
	});
});
