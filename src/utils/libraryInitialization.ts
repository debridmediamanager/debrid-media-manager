...
import { handleSelectTorrent, selectShown, resetSelection } from '@/utils/librarySelection';
...
export async function selectPlayableFiles(
  ...
  const [results, errors] = await Promise.allSettled(waitingForSelection.map(f => f()));
