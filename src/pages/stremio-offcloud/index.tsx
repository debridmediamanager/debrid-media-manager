...
export function StremioOffcloudPage() {
	const dmmCastToken = useOffcloudCastToken({ enroll: true });
	const [hasOffcloudCredentials] = useState(() => {
		if (typeof window !== 'undefined') {
			// Offcloud has one credential and no OAuth: the account-page API key.
			return !!localStorage.getItem('oc:apiKey');
		}
		return false;
	});

	if (!hasOffcloudCredentials) {
		return (
			<div className="flex min-h-screen flex-col items-center justify-center bg-gray-900 p-4">
				<Head>
					<title>DMM Cast for Offcloud - Stremio</title>
				</Head>
				<div className="max-w-md rounded-lg border-2 border-orange-500 bg-orange-900/20 p-6 text-center">
					...
					<h1 className="mb-3 text-2xl font-bold text-orange-400">Offcloud Required</h1>
	...
	return (
		<div className="flex min-h-screen flex-col items-center justify-center bg-gray-900 p-4">
			...
			<Image
				...
				className="mb-4"
			/>
			<h1 className="mb-4 text-2xl font-bold text-orange-400">DMM Cast for Offcloud</h1>
			<div className="flex flex-col items-center text-white">
					...
					<div className="mb-4 mt-4 h-max text-center leading-8">
						<Link
							href={`stremio://${window.location.origin.replace(
								/^https?:\/\//,
								''
							)}/api/stremio-oc/${dmmCastToken}/manifest.json`}
						...
						<Link
							href={`https://web.stremio.com/#/addons?addon=${encodeURIComponent(
								`${window.location.origin}/api/stremio-oc/${dmmCastToken}/manifest.json`
							)}`}
							className="text-md haptic-sm m-1 rounded border-2 border-orange-500 bg-orange-800/30 px-4 py-2 font-medium text-gray-100 transition-colors hover:bg-orange-700/50"
							target="_blank"
						...
						<div className="mt-2 text-sm text-orange-400">
							<AlertTriangle className="mr-1 inline-block h-4 w-4 text-orange-400" />
						...
						<div className="mt-4 border-t border-gray-700 pt-4">
							...
							<Link
								href={`stremio://${window.location.origin.replace(
									/^https?:\/\//,
									''
								)}/api/stremio-oc/${dmmCastToken}/no-catalog/manifest.json`}
							...
							<Link
								href={`https://web.stremio.com/#/addons?addon=${encodeURIComponent(
									`${window.location.origin}/api/stremio-oc/${dmmCastToken}/no-catalog/manifest.json`
								)}`}
								className="haptic-sm m-1 rounded border border-gray-600 bg-gray-800/50 px-3 py-1 text-sm font-medium text-gray-300 transition-colors hover:bg-gray-700/50"
								target="_blank"
								rel="noopener noreferrer"
				...
				<div className="space-y-2 text-gray-300">
					...
					<div>
						2. Click the &quot;
						<Cast className="inline-block h-3 w-3 text-orange-400" /> Cast&quot; button
					</div>
					...
					<div>
						...
						<span className="inline-flex items-center">
							Enjoy! <Popcorn className="ml-1 inline-block h-4 w-4 text-orange-500" />
						</span>
					</div>
				</div>

				<CastSettingsPanel service="oc" accentColor="orange" />
			</div>

			<div className="mt-6 flex gap-4">
				{dmmCastToken && (
					<Link
						href="/stremio-offcloud/manage"
...
export default dynamic(() => Promise.resolve(withAuth(StremioOffcloudPage)), { ssr: false });
