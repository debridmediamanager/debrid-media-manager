...
export function OffcloudManagePage() {
	// Offcloud has one credential and no OAuth: the account-page API key.
	const apiKey = useOffcloudApiKey();
	useOffcloudCastToken({ enroll: true });
	...
	useEffect(() => {
		const fetchLinks = async () => {
			...
			try {
				...
				const response = await fetch('/api/stremio-oc/links', {
					method: 'POST',
		...
		const loadMetadata = async () => {
			const results = await Promise.all(
				imdbIdsToFetch.map(async (imdbId) => {
					...
					try {
						console.info(
							`[OffcloudStremioManage] Fetching metadata for ${imdbId} using ${endpoint}`
						);
						...
						console.error(
							`[OffcloudStremioManage] Metadata lookup failed for ${imdbId}:`,
							error
						);
						return {
							imdbId,
	...
	const handleDelete = async (link: CastedLink) => {
		try {
			...
			const response = await fetch('/api/stremio-oc/deletelink', {
				method: 'DELETE',
				headers: {
	...
	return (
		<div className="flex min-h-screen flex-col items-center bg-gray-900 p-4">
			<Head>
				<title>DMM Cast Offcloud - Manage Casted Links</title>
			</Head>
			<Image
				width={100}
				height={100}
			...
			<div className="mb-6 flex flex-col items-center gap-4">
				<h1 className="text-xl font-bold text-white">
					DMM Cast Offcloud - Manage Casted Links
				</h1>
			</div>
						...
						return (
							<div key={imdbId} className="rounded-lg bg-gray-800 p-4">
								<div className="mb-4 flex items-center justify-between">
									...
									<Link
										href={`/x/${imdbId}`}
										className="haptic-sm rounded bg-orange-600 px-3 py-1 text-sm text-white hover:bg-orange-700"
											...
											>
												<div className="mr-2 flex min-w-0 flex-1 items-center gap-2">
													<input
														...
														className="h-4 w-4 rounded border-gray-300 bg-gray-600 text-orange-600 focus:ring-orange-500"
													/>
			...
			>
				Back to Stremio Offcloud
			</Link>
		</div>
	);
}

export default dynamic(() => Promise.resolve(withAuth(OffcloudManagePage)), { ssr: false });
