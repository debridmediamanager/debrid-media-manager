...
export function PremiumizeManagePage() {
	...
	useEffect(() => {
		const fetchLinks = async () => {
			...
			try {
				...
				const response = await fetch('/api/stremio-pm/links', {
					method: 'POST',
		...
		const loadMetadata = async () => {
			const results = await Promise.all(
				imdbIdsToFetch.map(async (imdbId) => {
					...
					try {
						console.info(
							`[PremiumizeStremioManage] Fetching metadata for ${imdbId} using ${endpoint}`
						);
						...
						console.error(
							`[PremiumizeStremioManage] Metadata lookup failed for ${imdbId}:`,
							error
						);
						return {
							imdbId,
	...
	const handleDelete = async (link: CastedLink) => {
		try {
			...
			const response = await fetch('/api/stremio-pm/deletelink', {
				method: 'DELETE',
				headers: {
	...
	return (
		<div className="flex min-h-screen flex-col items-center bg-gray-900 p-4">
			<Head>
				<title>DMM Cast Premiumize - Manage Casted Links</title>
			</Head>
			<Image
				width={100}
				height={100}
			...
			<div className="mb-6 flex flex-col items-center gap-4">
				<h1 className="text-xl font-bold text-white">
					DMM Cast Premiumize - Manage Casted Links
				</h1>
			</div>
								...
								<div className="mb-4 flex items-center justify-between">
									<label className="flex cursor-pointer items-center gap-2">
										<input
											...
											className="h-4 w-4 rounded border-gray-300 bg-gray-700 text-yellow-600 focus:ring-yellow-500"
										/>
											...
											>
												<div className="mr-2 flex min-w-0 flex-1 items-center gap-2">
													<input
														...
														className="h-4 w-4 rounded border-gray-300 bg-gray-600 text-yellow-600 focus:ring-yellow-500"
													/>
			...
			>
				Back to Stremio Premiumize
			</Link>
		</div>
	);
}

export default dynamic(() => Promise.resolve(withAuth(PremiumizeManagePage)), { ssr: false });
