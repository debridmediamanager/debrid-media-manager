...
export function StremioPremiumizePage() {
	const dmmCastToken = usePremiumizeCastToken({ enroll: true });
	const [hasPremiumizeCredentials] = useState(() => {
		if (typeof window !== 'undefined') {
			// Either credential works: the OAuth device-code token or the API key
			return !!(localStorage.getItem('pm:accessToken') || localStorage.getItem('pm:apiKey'));
		}
		return false;
	});

	if (!hasPremiumizeCredentials) {
		return (
			<div className="flex min-h-screen flex-col items-center justify-center bg-gray-900 p-4">
				<Head>
					<title>DMM Cast for Premiumize - Stremio</title>
				</Head>
				<div className="max-w-md rounded-lg border-2 border-red-500 bg-red-900/20 p-6 text-center">
					<AlertTriangle className="mx-auto mb-4 h-12 w-12 text-red-400" />
					<h1 className="mb-3 text-2xl font-bold text-red-400">Premiumize Required</h1>
	...
	return (
		<div className="flex min-h-screen flex-col items-center justify-center bg-gray-900 p-4">
			...
			<Image
				...
				className="mb-4"
			/>
			<h1 className="mb-4 text-2xl font-bold text-red-400">DMM Cast for Premiumize</h1>
			<div className="flex flex-col items-center text-white">
					...
					<div className="mb-4 mt-4 h-max text-center leading-8">
						<Link
							href={`stremio://${window.location.origin.replace(
								/^https?:\/\//,
								''
							)}/api/stremio-pm/${dmmCastToken}/manifest.json`}
							className="text-md haptic-sm m-1 rounded border-2 border-red-500 bg-red-800/30 px-4 py-2 font-medium text-gray-100 transition-colors hover:bg-red-700/50"
						>
							<Wand2 className="mr-1 inline-block h-4 w-4 text-red-400" />
							Install
						</Link>
						<Link
							href={`https://web.stremio.com/#/addons?addon=${encodeURIComponent(
								`${window.location.origin}/api/stremio-pm/${dmmCastToken}/manifest.json`
							)}`}
							className="text-md haptic-sm m-1 rounded border-2 border-red-500 bg-red-800/30 px-4 py-2 font-medium text-gray-100 transition-colors hover:bg-red-700/50"
							target="_blank"
						...
						<div className="mt-4 border-t border-gray-700 pt-4">
							...
							<Link
								href={`stremio://${window.location.origin.replace(
									/^https?:\/\//,
									''
								)}/api/stremio-pm/${dmmCastToken}/no-catalog/manifest.json`}
							...
							<Link
								href={`https://web.stremio.com/#/addons?addon=${encodeURIComponent(
									`${window.location.origin}/api/stremio-pm/${dmmCastToken}/no-catalog/manifest.json`
								)}`}
								className="haptic-sm m-1 rounded border border-gray-600 bg-gray-800/50 px-3 py-1 text-sm font-medium text-gray-300 transition-colors hover:bg-gray-700/50"
								target="_blank"
								rel="noopener noreferrer"
				...
				<div className="space-y-2 text-gray-300">
					...
					<div>
						2. Click the &quot;
						<Cast className="inline-block h-3 w-3 text-red-400" /> Cast&quot; button
					</div>
					...
					<div>
						...
						<span className="inline-flex items-center">
							Enjoy! <Popcorn className="ml-1 inline-block h-4 w-4 text-red-500" />
						</span>
					</div>
				</div>

				<CastSettingsPanel service="pm" accentColor="red" />
			</div>

			<div className="mt-6 flex gap-4">
				{dmmCastToken && (
					<Link
						href="/stremio-premiumize/manage"
						className="haptic-sm rounded border-2 border-red-500 bg-red-800/30 px-4 py-2 text-sm font-medium text-red-100 transition-colors hover:bg-red-700/50"
					>
						<span className="inline-flex items-center">
...
export default dynamic(() => Promise.resolve(withAuth(StremioPremiumizePage)), { ssr: false });
