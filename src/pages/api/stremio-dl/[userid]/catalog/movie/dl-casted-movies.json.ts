...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	try {
		const movies = await db.fetchDebridLinkCastedMovies(userid);
		res.status(200).json({
		...
		console.error(
			'Failed to get Debrid-Link casted movies:',
			error instanceof Error ? error.message : 'Unknown error'
		);
		res.status(500).json({ error: 'Failed to get Debrid-Link casted movies' });
	}
}
