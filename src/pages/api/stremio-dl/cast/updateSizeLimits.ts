...
// no Debrid-Link call is needed and the credential never leaves the browser.
// That matters more here than elsewhere: every avoided request is one that
// cannot contribute to an hour-long endpoint lockout.
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	try {
		const updated = await db.updateDebridLinkCastSettings(
			userId,
		...
		// 404 lets the client fall back to a full save with the credential in hand
		if (!updated) {
			res.status(404).json({ status: 'error', errorMessage: 'No Debrid-Link cast profile' });
			return;
		}
		...
		console.error(
			'Error updating Debrid-Link cast settings:',
