...
async function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	if (!profile.hideCastOption) {
		streams.push({
			...
			externalUrl,
			behaviorHints: { bingeGroup: `dmm-oc:${imdbidStr}:cast` },
		});
	}
	...
	try {
		...
		const [userCastItems, otherItems, troveCandidates] = await Promise.all([
			db.getOffcloudUserCastStreams(imdbidStr, userid, 5),
			db.getOffcloudOtherStreams(
				imdbidStr,
				userid,
				otherStreamsLimit,
			...
			// title, offered once Offcloud confirms the hash is cached. This is
		...
		// Offcloud can answer "will this actually play?" before offering it:
		// `POST /api/cache` is free, non-destructive and batched (5,000 hashes in
		// 2.1 s), so a hash that has fallen out of the cache is dropped here
		...
		if (uniqueHashes.length > 0) {
			try {
				const results = await checkOffcloudCache(profile.apiKey, uniqueHashes);
				...
				console.error(
					'[Stremio-OC Stream] Cache probe failed, offering casts unfiltered:',
					error instanceof Error ? error.message : 'Unknown error'
				);
		...
		console.log('[Stremio-OC Stream] Stream stats:', {
		...
		const push = (
			...
			streams.push({
				...
				// The hash and the stored path, never a link: an Offcloud CDN URL
				// carries the caster's account token.
				url: `${process.env.DMM_ORIGIN}/api/stremio-oc/${userid}/play/${item.hash}?file=${encodeURIComponent(item.path ?? item.filename ?? '')}`,
