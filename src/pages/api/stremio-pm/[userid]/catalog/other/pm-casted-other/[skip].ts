import { PAGE_SIZE, getPremiumizeDMMLibrary } from '@/utils/premiumizeCastCatalogHelper';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	// `.../pm-casted-other/skip=24.json`. It is an item offset, not a page.
	...
	try {
		const result = await getPremiumizeDMMLibrary(userid, page);
		if ('error' in result) {
