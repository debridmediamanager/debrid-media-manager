...
const eztvUrl = (imdbId: string, page: number = 1) => `https://eztv.tf/api/get-torrents?imdb_id=${imdbId.replace('tt', '')}&limit=100&page=${page}`;
	...
	while (true) {
		for (const imdbId of imdbIds) {
			...
			try {
				...
				let eztvLinks: ScrapeSearchResult[] = (response.data.torrents || [])
					.map((e: EztvItem) => processStream(e, imdbId));
				...
				while (eztvLinks.length < totalCount) {
					...
					eztvLinks = eztvLinks.concat((nextResponse.data.torrents || [])
						.map((e: EztvItem) => processStream(e, imdbId)));
				}
