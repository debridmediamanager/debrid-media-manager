...
export function AllDebridManagePage() {
		...
		const loadMetadata = async () => {
			const results = await Promise.all(
				imdbIdsToFetch.map(async (imdbId) => {
					...
					try {
						console.info(
							`[AllDebridStremioManage] Fetching metadata for ${imdbId} using ${endpoint}`
						);
						...
						console.error(
							`[AllDebridStremioManage] Metadata lookup failed for ${imdbId}:`,
							error
						);
						return {
							imdbId,
	...
	const handleDelete = async (link: CastedLink) => {
		try {
			...
			const response = await fetch('/api/stremio-ad/deletelink', {
				method: 'DELETE',
				headers: {
	...
	return (
		<div className="flex min-h-screen flex-col items-center bg-gray-900 p-4">
			<Head>
				<title>DMM Cast AllDebrid - Manage Casted Links</title>
			</Head>
			<Image
				width={100}
				height={100}
			...
			<div className="mb-6 flex flex-col items-center gap-4">
				<h1 className="text-xl font-bold text-white">
					DMM Cast AllDebrid - Manage Casted Links
				</h1>
			</div>
			...
			>
				Back to Stremio AllDebrid
			</Link>
		</div>
	);
}

export default dynamic(() => Promise.resolve(withAuth(AllDebridManagePage)), { ssr: false });
