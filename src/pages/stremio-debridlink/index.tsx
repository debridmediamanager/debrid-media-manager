...
export function StremioDebridLinkPage() {
	const dmmCastToken = useDebridLinkCastToken({ enroll: true });
	const [hasDebridLinkCredentials] = useState(() => {
		if (typeof window !== 'undefined') {
			// Either credential works: an OAuth access token from the device-code
			// login, or a token pasted from the account page. Both go in the same
			// Bearer header.
	...
	if (!hasDebridLinkCredentials) {
		return (
			<div className="flex min-h-screen flex-col items-center justify-center bg-gray-900 p-4">
				<Head>
					<title>DMM Cast for Debrid-Link - Stremio</title>
				</Head>
				<div className="max-w-md rounded-lg border-2 border-sky-500 bg-sky-900/20 p-6 text-center">
					<AlertTriangle className="mx-auto mb-4 h-12 w-12 text-sky-400" />
					<h1 className="mb-3 text-2xl font-bold text-sky-400">Debrid-Link Required</h1>
	...
	return (
		<div className="flex min-h-screen flex-col items-center justify-center bg-gray-900 p-4">
			...
			<Image
				...
				className="mb-4"
			/>
			<h1 className="mb-4 text-2xl font-bold text-sky-400">DMM Cast for Debrid-Link</h1>
					...
					<div className="mb-4 mt-4 h-max text-center leading-8">
						<Link
							href={`stremio://${window.location.origin.replace(
								/^https?:\/\//,
								''
							)}/api/stremio-dl/${dmmCastToken}/manifest.json`}
						...
						<Link
							href={`https://web.stremio.com/#/addons?addon=${encodeURIComponent(
								`${window.location.origin}/api/stremio-dl/${dmmCastToken}/manifest.json`
							)}`}
							className="text-md haptic-sm m-1 rounded border-2 border-sky-500 bg-sky-800/30 px-4 py-2 font-medium text-gray-100 transition-colors hover:bg-sky-700/50"
							target="_blank"
						...
						<div className="mt-2 text-sm text-sky-400">
							<AlertTriangle className="mr-1 inline-block h-4 w-4 text-sky-400" />
						...
						<div className="mt-4 border-t border-gray-700 pt-4">
							...
							<Link
								href={`stremio://${window.location.origin.replace(
									/^https?:\/\//,
									''
								)}/api/stremio-dl/${dmmCastToken}/no-catalog/manifest.json`}
							...
							<Link
								href={`https://web.stremio.com/#/addons?addon=${encodeURIComponent(
									`${window.location.origin}/api/stremio-dl/${dmmCastToken}/no-catalog/manifest.json`
								)}`}
								className="haptic-sm m-1 rounded border border-gray-600 bg-gray-800/50 px-3 py-1 text-sm font-medium text-gray-300 transition-colors hover:bg-gray-700/50"
								target="_blank"
								rel="noopener noreferrer"
				...
				<div className="space-y-2 text-gray-300">
					...
					<div>
						...
						<span className="inline-flex items-center">
							Enjoy! <Popcorn className="ml-1 inline-block h-4 w-4 text-sky-500" />
						</span>
					</div>
				</div>

				<div className="mt-4 max-w-md rounded border border-sky-700 bg-sky-900/20 p-3 text-center text-xs text-sky-100">
					Debrid-Link has no cache check, so casting a release adds it to your seedbox.
					One that is already cached is ready in about a second; one that is not starts
					downloading for real and uses one of your 50 torrents for the day.
				</div>

				<CastSettingsPanel service="dl" accentColor="sky" />
			</div>

			<div className="mt-6 flex gap-4">
				{dmmCastToken && (
					<Link
						href="/stremio-debridlink/manage"
...
export default dynamic(() => Promise.resolve(withAuth(StremioDebridLinkPage)), { ssr: false });
