...
export function DebridLinkManagePage() {
	// Either an OAuth access token or a pasted API token - both authenticate the
	// same Bearer header, and the token wins because it is the narrower one.
	const apiKey = useDebridLinkCredential();
	useDebridLinkCastToken({ enroll: true });
	...
	useEffect(() => {
		const fetchLinks = async () => {
			...
			try {
				// The endpoint is POST-only and reads the credential from the
				// body, so it never lands in an access log - which matters here
				// because Debrid-Link also accepts `?access_token=`, and a token
				// in a URL is a token on disk.
				const response = await fetch('/api/stremio-dl/links', {
					method: 'POST',
		...
		const loadMetadata = async () => {
			const results = await Promise.all(
				imdbIdsToFetch.map(async (imdbId) => {
					...
					try {
						console.info(
							`[DebridLinkStremioManage] Fetching metadata for ${imdbId} using ${endpoint}`
						);
						...
						console.error(
							`[DebridLinkStremioManage] Metadata lookup failed for ${imdbId}:`,
							error
						);
						return {
							imdbId,
	...
	const handleDelete = async (link: CastedLink) => {
		try {
			...
			const response = await fetch('/api/stremio-dl/deletelink', {
				method: 'DELETE',
				headers: {
	...
	return (
		<div className="flex min-h-screen flex-col items-center bg-gray-900 p-4">
			<Head>
				<title>DMM Cast Debrid-Link - Manage Casted Links</title>
			</Head>
			<Image
				width={100}
				height={100}
			...
			<div className="mb-6 flex flex-col items-center gap-4">
				<h1 className="text-xl font-bold text-white">
					DMM Cast Debrid-Link - Manage Casted Links
				</h1>
			</div>
			...
			>
				Back to Stremio Debrid-Link
			</Link>
		</div>
	);
}

export default dynamic(() => Promise.resolve(withAuth(DebridLinkManagePage)), { ssr: false });
