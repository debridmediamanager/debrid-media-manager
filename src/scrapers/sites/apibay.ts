...
const apibayHost = process.env.APIBAYHOST ?? 'https://apibay.org';
...
const createSearchUrl = (finalQuery: string) =>
	`${apibayHost}/search/${encodeURIComponent(finalQuery)}/`;
...
async function processItem(
	...
	while (true) {
		try {
			const response = await axios.get(`${apibayHost}/information/${item.hash}`, {
				timeout: 60000,
			});
			responseData = response.data;
			break;
		} catch (error: any) {
			console.log('request error:', error.message, `${apibayHost}/information/${item.hash}`);
			retries++;
...
async function processInBatches(
	...
	while (i < promises.length) {
		...
		if (percentageIncrease >= 20) {
			console.log(`🏴‍☠ ApiBay batch ${i}/${promises.length}:${title}`);
			lastPrintedIndex = i;
		}
...
const processPage = async (
	...
	console.log(`🏴‍☠ ApiBay search returned ${titlesAndHashes.length} for ${finalQuery}`);
...
export async function scrapeApiBay(
	finalQuery: string,
	targetTitle: string,
	years: string[],
	airDate: string
	...
	try {
		...
		console.error('scrapeApiBay page processing error', error);
	}
	return [];
}
...
export function processStream(item: ApiBayItem, imdbId: string): ScrapeSearchResult | null {
	if (item.imdb !== imdbId) {
		return null;
	}
	return {
