...
const x1337xhost = process.env.X1337X ?? 'https://1337x.to';

const createSearchUrl = (finalQuery: string) =>
	`${x1337xhost}/search/${encodeURIComponent(finalQuery)}/`;
...
async function processItem(
	...
	while (true) {
		try {
			const response = await axios.get(`${x1337xhost}/information/${item.hash}`, {
				timeout: 60000,
			});
			responseData = response.data;
			break;
		} catch (error: any) {
			console.log('request error:', error.message, `${x1337xhost}/information/${item.hash}`);
			retries++;
