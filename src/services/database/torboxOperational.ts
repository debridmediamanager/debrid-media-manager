...
export interface TorBoxOperationStats {
	operation: TorBoxOperation;
	totalCount: number;
	successCount: number;
	failureCount: number;
	successRate: number;
...
export interface TorBoxOverallStats {
	totalCount: number;
	successCount: number;
	failureCount: number;
	successRate: number;
	isDown: boolean;
	byOperation: Record<TorBoxOperation, TorBoxOperationStats>;
	lastHour: Date | null;
}

const MONITORED_OPERATIONS: TorBoxOperation[] = [
	'GET /user/me',
	'GET /torrents/mylist',
	'GET /torrents/checkcached',
	'GET /torrents/requestdl',
...
// TorBox versions its API in the path (/v1/api/...), so every matcher tests the
// tail rather than the whole pathname - a future /v2 keeps reporting into the
// same buckets instead of silently going unrecorded.
const OPERATION_DEFINITIONS: Array<{
...
}> = [
	{
		...
		method: 'GET',
		test: (pathname) => pathname.endsWith('/user/me'),
	},
	{
		operation: 'GET /torrents/mylist',
		method: 'GET',
		test: (pathname) => pathname.endsWith('/torrents/mylist'),
	},
	{
		operation: 'GET /torrents/checkcached',
		method: 'GET',
		test: (pathname) => pathname.endsWith('/torrents/checkcached'),
	},
	{
		operation: 'GET /torrents/requestdl',
		method: 'GET',
		test: (pathname) => pathname.endsWith('/torrents/requestdl'),
	},
	{
		operation: 'GET /torrents/torrentinfo',
		method: 'GET',
		test: (pathname) => pathname.endsWith('/torrents/torrentinfo'),
	},
	{
		operation: 'POST /torrents/torrentinfo',
		method: 'POST',
		test: (pathname) => pathname.endsWith('/torrents/torrentinfo'),
	},
	{
		operation: 'POST /torrents/createtorrent',
		method: 'POST',
		test: (pathname) => pathname.endsWith('/torrents/createtorrent'),
	},
	{
		operation: 'POST /torrents/controltorrent',
		method: 'POST',
		test: (pathname) => pathname.endsWith('/torrents/controltorrent'),
	},
	{
		operation: 'GET /webdl/mylist',
		method: 'GET',
		test: (pathname) => pathname.endsWith('/webdl/mylist'),
	},
	{
		operation: 'GET /webdl/requestdl',
		method: 'GET',
		test: (pathname) => pathname.endsWith('/webdl/requestdl'),
	},
	{
		operation: 'POST /webdl/createwebdownload',
		method: 'POST',
		test: (pathname) => pathname.endsWith('/webdl/createwebdownload'),
	},
	{
		operation: 'POST /webdl/controlwebdownload',
		method: 'POST',
		test: (pathname) => pathname.endsWith('/webdl/controlwebdownload'),
	},
];

export function resolveTorBoxOperation(
...
export class TorBoxOperationalService extends DatabaseClient {
	/**
	 * Records a TorBox API operation by atomically incrementing the hourly counter.
	 * This is a fire-and-forget operation - errors are logged but not thrown.
	 */
	public async recordOperation(operation: TorBoxOperation, status: number): Promise<void> {
		const hour = getHourStart();
		// Only 5xx counts against TorBox. A 4xx is almost always the caller's
		// key or request - and TorBox answers a rejected key with HTTP 200
		// success:false anyway, so neither belongs in the outage verdict.
		...
		try {
			...
			await this.prisma.torBoxOperationalHourly.upsert({
	 ...
	 */
	public async getStats(hoursBack: number = 24): Promise<TorBoxOverallStats> {
		...
		try {
			...
			const hourlyData = await this.prisma.torBoxOperationalHourly.findMany({
				where: {
					hour: { gte: cutoff },
				},
				orderBy: { hour: 'desc' },
			...
			const byOperation: Record<TorBoxOperation, TorBoxOperationStats> = {} as any;
			...
			for (const row of hourlyData) {
				const operation = row.operation as TorBoxOperation;
			...
			console.error('Failed to get TorBox operational stats:', error);
			return emptyStats;
		}
	}
	...
	> {
		try {
			...
			console.error('Failed to get TorBox operational hourly history:', error);
			return [];
		}
	}
	...
	public async rollupDaily(targetDate?: Date): Promise<boolean> {
		...
		try {
			...
			for (const [operation, data] of byOperation) {
				...
				await this.prisma.torBoxOperationalDaily.upsert({
					where: {
			...
			console.error('Failed to rollup TorBox operational daily:', error);
			return false;
		}
	}
	...
	> {
		try {
			...
			const dailyData = await this.prisma.torBoxOperationalDaily.findMany({
				where: {
					date: { gte: cutoff },
				},
				orderBy: { date: 'asc' },
			...
			console.error('Failed to get TorBox operational daily history:', error);
			return [];
		}
	}

	/**
	 * Cleans up hourly and daily data older than 90 days.
	 */
	public async cleanupOldData(): Promise<{ hourlyDeleted: number; dailyDeleted: number }> {
		...
		try {
			...
			const [hourlyResult, dailyResult] = await Promise.all([
				this.prisma.torBoxOperationalHourly.deleteMany({
					where: { hour: { lt: hourlyCutoff } },
				}),
				this.prisma.torBoxOperationalDaily.deleteMany({
			...
			console.error('Failed to cleanup old TorBox operational data:', error);
		}

		return { hourlyDeleted, dailyDeleted };
	}

	private buildEmptyStats(): TorBoxOverallStats {
		const byOperation: Record<TorBoxOperation, TorBoxOperationStats> = {} as any;
		for (const op of MONITORED_OPERATIONS) {
			byOperation[op] = {
				operation: op,
