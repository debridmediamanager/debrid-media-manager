...
import { saveOffcloudCastProfile, updateOffcloudCastSettings } from '@/utils/offcloudCastApiClient';
import { defaultEpisodeSize, defaultMovieSize, defaultOtherStreamsLimit } from '@/utils/settings';
import { useEffect, useRef } from 'react';
import toast from 'react-hot-toast';
import { useOffcloudApiKey } from './auth';
import useLocalStorage from './localStorage';

interface UseOffcloudCastTokenOptions {
	/**
	 * Create the cast profile when there isn't one yet. Only the DMM Cast page
	 * should do this: enrolling costs an Offcloud call with the member's key.
	 * Everywhere else the hook resyncs an existing profile and nothing more.
	 */
	enroll?: boolean;
}

export function useOffcloudCastToken({ enroll = false }: UseOffcloudCastTokenOptions = {}) {
	const apiKey = useOffcloudApiKey();
	const [dmmCastToken, setDmmCastToken] = useLocalStorage<string>('oc:castToken');
	...
	const [syncedApiKey, setSyncedApiKey] = useLocalStorage<string>('oc:castSyncedKey');
	...
	useEffect(() => {
		...
		const sync = async () => {
			try {
				...
				if (keyIsCurrent) {
					const ok = await updateOffcloudCastSettings(dmmCastToken, settings);
					if (!ok) token = await saveOffcloudCastProfile(apiKey, settings);
				} else {
					token = await saveOffcloudCastProfile(apiKey, settings);
				}
				...
				toast.error('Failed to initialize DMM Cast for Offcloud.');
			} finally {
